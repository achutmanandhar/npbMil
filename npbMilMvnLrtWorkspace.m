...
for nIteration=1%:10
    ...
    myOptions.batchSizeFactorOfN = 5;
    myOptions.computeLogPred    = true;    
...
%     profile on;
tic;
    [yOut, xValTrainedClassifier, crossValKeysByBag] = nValMil(bags,numOfFolds,myTrainFun,myTestFun,myOptions);
%     [yOut, xValTrainedClassifier, crossValKeysByBag, timeTrain, timeTest] = nValMil(bags,numOfFolds,myTrainFun,myTestFun,myOptions); % leave-one-bag-out xVal    
%     profile report;
%     profile off; 
...
% dataSetName = 'ehd';
% folderName2 = 'roc10xEhd6Pc';
% folderName3 = 'NpbMilRndInitNby5';
% 
% nIteration = 1;
% load(fullfile(pathToSave,folderName1,'savedVariables',dataSetName,strcat(folderName2,folderName3),sprintf('timeTrain%d',nIteration)));
% load(fullfile(pathToSave,folderName1,'savedVariables',dataSetName,strcat(folderName2,folderName3),sprintf('timeTest%d',nIteration)));
% load(fullfile(pathToSave,folderName1,'savedVariables',dataSetName,strcat(folderName2,folderName3),sprintf('Pd%d',nIteration)));
% load(fullfile(pathToSave,folderName1,'savedVariables',dataSetName,strcat(folderName2,folderName3),sprintf('Pfa%d',nIteration)));
% figure(41);plot(Pfa,Pd,'b');hold on;
% grid on;
% legend('Online S=N/20','Online S=N/15','Online S=N/10','Online S=N/5','Batch S=N');
% axis([0 .25 .75 1]);
% axis([0 1 0 1]);
% xlabel('Pfa','FontSize',14);
% ylabel('Pd','FontSize',14);
% title('ROC performance on EHD landmine data (10-fold xVal)','FontSize',12);
